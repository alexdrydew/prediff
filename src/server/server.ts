/**
 * Per-workspace daemon: Bun.serve JSON API + SSE + static UI + source watcher +
...
import { loadPatchSource, savePatchSource } from "../store/patch";
...
export interface DaemonOptions {
  ...
  stateDir: string;
  range: string;
  /** Initial patch text when bootstrapping a daemon outside Git. */
  initialPatch?: string;
...
export class Daemon {
  ...
  private range: ResolvedRange | null = null;
  private patch: ParsedPatch | null = null;
  private sourceKind: "git" | "patch" = "git";
  ...
  constructor(private readonly opts: DaemonOptions) {
    ...
    this.watcher = new RepoWatcher(opts.repoRoot, () => {
      if (
        this.sourceKind === "git" &&
        this.range !== null &&
        (this.range.targetRef === null || this.range.targetRef === ":index")
      ) {
        void this.refresh();
      }
    });
  ...
  async start(): Promise<void> {
    if (this.opts.initialPatch !== undefined) {
      await this.openPatchSession(this.opts.initialPatch, null);
    } else {
  ...
  private async openSession(
    ...
    if (existing && existing.source_kind !== "patch" && existing.range === rangeSpec) {
      this.session = existing;
      this.session.source_kind = "git";
      if (this.applyScope(scope, scopeFiles)) await this.store.save(this.session);
    } else {
      this.session = await this.store.create(
        this.opts.repoRoot,
        rangeSpec,
        scope,
        scopeFiles,
    ...
    [this.manifest, this.rawDiff] = await Promise.all([
      computeManifest(this.opts.repoRoot, range, this.session.revision),
      computeRawDiff(this.opts.repoRoot, range),
    ]);
  ...
  /**
   * Create or update the current patch-backed session. Re-opening with a
   * changed patch is a new revision of the same review; switching from Git
   * creates a fresh autogenerated session for this workspace.
   */
  private async openPatchSession(
    rawPatch: string,
    scope: string | null,
    scopeFiles: string[] | null = null,
    reopenReady = true,
    ...
    const existing = await this.store.loadCurrent();
    const reuse = existing?.source_kind === "patch";
    this.session = reuse
      ? existing
      : await this.store.create(this.opts.repoRoot, "patch", scope, scopeFiles, "patch");
    this.session.source_kind = "patch";
    this.sourceKind = "patch";
    this.range = null;

    const previousRaw = reuse
      ? await loadPatchSource(this.opts.stateDir, this.session.session_id)
      : null;
    const parsed = parsePatch(rawPatch, this.session.revision);
    this.stats.manifest_computes++;

    if (this.applyScope(scope, scopeFiles)) await this.store.save(this.session);

    let changed = false;
    if (previousRaw !== null && previousRaw !== parsed.raw) {
      const previous = parsePatch(previousRaw, this.session.revision);
      this.patch = parsed;
      this.manifest = previous.manifest;
      this.rawDiff = previous.raw;
      await savePatchSource(this.opts.stateDir, this.session.session_id, parsed.raw);
      changed = await this.applySnapshot(parsed.manifest, parsed.raw);
    } else {
      this.patch = parsed;
      this.manifest = parsed.manifest;
      this.rawDiff = parsed.raw;
      await savePatchSource(this.opts.stateDir, this.session.session_id, parsed.raw);
      this.applyScopeFlags();
      this.fileDiffCache.clear();
      await this.persistRevision();
    ...
    this.manifestSignature = patchSignature(parsed.raw);
    if (!reuse) {
      this.hub.broadcast("session.changed", { session_id: this.session.session_id });
    }
    if (reopenReady && this.session.session_state === "ready") {
  ...
  /** Select a persisted session explicitly (`--session`) within this daemon. */
  private async selectSession(sessionId: string): Promise<void> {
    if (this.session.session_id === sessionId) return;
    const session = await this.store.load(sessionId);
    if (!session) throw new Error(`session not found: ${sessionId}`);
    this.session = session;

    if (session.source_kind === "patch") {
      const raw = await loadPatchSource(this.opts.stateDir, sessionId);
      if (raw === null) throw new Error(`patch source missing for session: ${sessionId}`);
      this.sourceKind = "patch";
      this.range = null;
      this.patch = parsePatch(raw, session.revision);
      this.manifest = this.patch.manifest;
      this.rawDiff = this.patch.raw;
      this.manifestSignature = patchSignature(this.rawDiff);
    } else {
      this.sourceKind = "git";
      this.patch = null;
      this.range = await resolveRange(this.opts.repoRoot, session.range);
      const [next, raw, previous] = await Promise.all([
        computeManifest(this.opts.repoRoot, this.range, session.revision),
        computeRawDiff(this.opts.repoRoot, this.range),
        this.revisions.load(sessionId, session.revision),
      ]);
      if (previous !== null && previous.raw_diff !== raw) {
        this.manifest = previous.manifest;
        this.rawDiff = previous.raw_diff;
        await this.applySnapshot(next, raw);
      } else {
        this.manifest = next;
        this.rawDiff = raw;
      }
      this.manifestSignature = await this.watcher.computeSignature();
    }
    await this.store.setCurrent(sessionId);
    this.applyScopeFlags();
    this.fileDiffCache.clear();
    this.hub.broadcast("session.changed", { session_id: sessionId });
  }
  ...
  private async captureContents(revision: number): Promise<RevisionContents> {
    ...
    for (const f of this.manifest.files) {
      ...
      if (this.sourceKind === "patch") {
        skipped[f.path] = "patch source does not contain full file content";
        continue;
      }
      files[f.path] = await this.sourceContent("new", f.path);
    }
  ...
  private async doRefresh(): Promise<boolean> {
    if (this.sourceKind === "patch") return false;
    const signature = await this.watcher.computeSignature();
    ...
    return this.applySnapshot(next, raw);
  }

  /** Install a changed source snapshot and preserve the review lifecycle. */
  private async applySnapshot(next: DiffManifest, raw: string): Promise<boolean> {
  ...
  private async sourceContent(side: Side, filePath: string): Promise<string[] | null> {
    if (this.sourceKind === "patch") {
      return this.patch ? patchSideContent(this.patch, side, filePath) : null;
    }
    if (this.range === null) return null;
    return sideContent(this.opts.repoRoot, this.range, side, filePath);
  }

  /** Full content is intentionally unavailable for snapshot-only patches. */
  private async fullSourceContent(side: Side, filePath: string): Promise<string[] | null> {
    return this.sourceKind === "patch" ? null : this.sourceContent(side, filePath);
  }
  ...
  private async reanchorComments(): Promise<void> {
    ...
    const content = async (file: string, side: Side): Promise<string[] | null> => {
      ...
      if (!contentCache.has(key)) {
        contentCache.set(key, await this.sourceContent(side, file));
      }
  ...
  private async dispatch(req: Request, url: URL): Promise<Response> {
    ...
    if (pathname === "/api/open" && method === "POST") {
      ...
      if (typeof patch === "string") {
        await this.openPatchSession(patch, scope, scopeFiles);
      } else if (this.sourceKind === "patch" && typeof body["range"] !== "string") {
        if (this.applyScope(scope, scopeFiles)) {
          this.applyScopeFlags();
          await this.store.save(this.session);
        }
        // The browser's ready-screen "Reopen review" call has no source body;
        // keep the current patch snapshot instead of trying to enter Git mode.
        ...
        const range = typeof body["range"] === "string" ? body["range"] : "working";
        if (this.sourceKind !== "git" || range !== this.session.range) {
    ...
    if (pathname === "/api/session/select" && method === "POST") {
      const body = await readBody(req);
      const sessionId = body["session_id"];
      if (typeof sessionId !== "string") {
        return json({ error: "required: session_id (string)" }, 400);
      }
      await this.selectSession(sessionId);
      return json({ session_id: this.session.session_id });
    }
    ...
    if (pathname === "/api/file" && method === "GET") {
      ...
      const lines = await this.fullSourceContent(side, filePath);
      if (lines === null) {
        const message =
          this.sourceKind === "patch"
            ? `full ${side}-side content unavailable for patch source: ${filePath}`
            : `no ${side}-side content: ${filePath}`;
        return json({ error: message }, this.sourceKind === "patch" ? 409 : 404);
      }
    ...
    if (pathname === "/api/refresh" && method === "POST") {
      const body = await readBody(req);
      if (typeof body["patch"] === "string" && this.sourceKind !== "patch") {
        return json(
          { error: "current review is Git-backed; use `prediff open -` to start a patch review" },
          409,
        );
      }
      const changed =
        typeof body["patch"] === "string"
          ? await this.openPatchSession(body["patch"], null, null, false)
          : await this.refresh();
      return json({
        changed,
        revision: this.session.revision,
  ...
  private async fileDiff(file: ManifestFile, force: boolean): Promise<FileDiff> {
    ...
    let result: FileDiff;
    if (this.sourceKind === "patch") {
      const parsed = this.patch?.files.get(file.path);
      if (!parsed) throw new Error(`patch section missing for: ${file.path}`);
      result =
        parsed.large && !force
          ? { ...parsed, hunks: [] }
          : parsed.large
      ...
      if (this.range === null) throw new Error("Git range is not initialized");
      result = await computeFileDiff(this.opts.repoRoot, this.range, file, { force });
    }
  ...
  private async interdiffSide(
    ...
    if (!changed) {
      ...
      const lines = await this.fullSourceContent("old", filePath);
      return lines === null && this.sourceKind === "patch"
        ? { ok: false, reason: "full content unavailable for patch source" }
        : { ok: true, lines };
    }
    if (contents !== null) {
  ...
  private async createComment(body: JsonBody): Promise<Response> {
    ...
    if (file === null) {
      ...
      if (line === 0) {
        ...
        const content = await this.sourceContent(side, file);
  ...
  private async reanchorComment(id: string, body: JsonBody): Promise<Response> {
    ...
    if (body["file_note"] === true) {
      ...
      if (endLine < line) return json({ error: "invalid line range" }, 400);
      const side: Side = body["side"] === "old" ? "old" : "new";
      const content = await this.sourceContent(side, comment.file);
      if (content === null) {
  ...
  private async suggestionFor(id: string): Promise<Response> {
    ...
    const content = await this.sourceContent(comment.side, comment.file);
...
function patchSignature(raw: string): string {
